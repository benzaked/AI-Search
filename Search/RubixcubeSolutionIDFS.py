from ScrambleRubixcube import make_move
import numpy as np
...
def idfs(start, print_res=False):
    cost_limit = 1
    nodes = 0
    frontier = list()
    ...
    while True:
        ...
        while len(frontier) != 0:
            ...
            if goal_reached(curr.cube):
                goal_height = curr.cost
                branching_factor = sum(branching_factors) / len(branching_factors) if len(branching_factors) > 0 else 0
                if print_res:
                    print('Goal Height:', goal_height)
                    print('Branching Factor:', branching_factor)
                    print("Nodes Generated:", nodes)
                return goal_height, branching_factor, nodes
...
def solve_DFS(x, num_scrambles, trial):
    curr = State()
    curr.cube = x
    handle = open(f'inputs/input_{num_scrambles}_trial_{trial}.txt')
    ...
    start = time.time()
    goal_height, branching_factor, nodes = idfs(curr)

    total_time = time.time() - start
    return goal_height, branching_factor, nodes, total_time
